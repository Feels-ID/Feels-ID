			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] BOT HARUS JADI ADMIN ❌',
                                        daftarB: `──「 BELUM REGISTRASI」──\nHalo  OMM !\nRegistrasi dulu ya ommmm... \n\nCommand : ${prefix}daftar  nama|umur\nContoh : ${prefix}daftar AGUSZ|20`,
				}
			}
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Tuh ownerku omm, jan lu culik >_<',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Kirim perintah ${prefix}snowwrite teks1|teks2`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'block':
					...
					client.sendMessage(from, `mampuss kena block ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
           ...
           break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'asekk dapet jatah kick ,otw kick.🥳🥳�', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'tutupgc':
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Lagu yang Ditemukan APBOT!!!*\nJudul : ${anu.result.title}\nLink : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*AUDIO SEDANG DI KIRIM, JAN SPAMM YAH OMM*`
